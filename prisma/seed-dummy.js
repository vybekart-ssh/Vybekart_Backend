...
async function seedProfilesAndRelatedData() {
  ...
  if (!categories.length) throw new Error('No categories found. Run reference seed first.');
  ...
  for (let i = 0; i < users.length; i += 1) {
    ...
    if (i % 2 === 0) {
      const seller = await prisma.seller.upsert({
        where: { userId: user.id },
        update: { businessName: `Dummy Store ${i + 1}`, status: VerificationStatus.VERIFIED },
        create: {
          id: `dummy-seller-${i + 1}`,
          userId: user.id,
      ...
      sellers.push({ id: seller.id, userId: user.id, businessName: seller.businessName });
    }

    await prisma.address.upsert({
      where: { id: `dummy-address-${i + 1}` },
      update: {},
      create: {
        id: `dummy-address-${i + 1}`,
        userId: user.id,
  ...
  for (let i = 0; i < sellers.length; i += 1) {
    ...
    await prisma.sellerCategory.upsert({
      where: { sellerId_categoryId: { sellerId: sellers[i].id, categoryId: primaryCategory.id } },
      update: {},
      create: { sellerId: sellers[i].id, categoryId: primaryCategory.id },
    });
    await prisma.sellerCategory.upsert({
      where: { sellerId_categoryId: { sellerId: sellers[i].id, categoryId: secondaryCategory.id } },
      update: {},
      create: { sellerId: sellers[i].id, categoryId: secondaryCategory.id },
    });
  }
  ...
  for (let i = 0; i < sellers.length; i += 1) {
    for (let p = 1; p <= 3; p += 1) {
      ...
      const product = await prisma.product.upsert({
        where: { id: productId },
        update: { stock: 30 + p, status: ProductStatus.ACTIVE, price },
        create: {
          id: productId,
  ...
  for (let i = 0; i < sellers.length; i += 1) {
    ...
    await prisma.stream.upsert({
      where: { id: streamId },
      update: { isLive: i % 2 === 0, viewCount: 100 + i * 20 },
      create: {
        id: streamId,
        sellerId: sellers[i].id,
        categoryId: category.id,
    ...
    for (let j = 0; j < sellerProducts.length; j += 1) {
      await prisma.streamProduct.upsert({
        where: { id: `dummy-stream-product-${i + 1}-${j + 1}` },
        update: { sortOrder: j },
        create: {
          id: `dummy-stream-product-${i + 1}-${j + 1}`,
          streamId,
  ...
  for (let i = 0; i < buyers.length; i += 1) {
    ...
    const status = [OrderStatus.DELIVERED, OrderStatus.PENDING, OrderStatus.SHIPPED][i % 3];
    await prisma.order.upsert({
      where: { id: orderId },
      update: { totalAmount: subtotal + 90, status },
      create: {
        id: orderId,
        buyerId: buyers[i].id,
