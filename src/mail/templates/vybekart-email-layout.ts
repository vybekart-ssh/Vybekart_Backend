...
/** Prefer ALPHA_LOGO_URL; else Supabase public bucket; else embedded SVG. */
export function resolveEmailLogoUrl(config: ConfigService): string {
  const explicit = config.get<string>('ALPHA_LOGO_URL')?.trim();
  if (explicit) return normalizeLogoUrlForEmail(explicit);

  const supabase = config.get<string>('SUPABASE_URL')?.trim().replace(/\/$/, '');
  if (supabase) {
    const bucket =
      config.get<string>('SUPABASE_PUBLIC_BUCKET')?.trim() || 'Vybekart';
    const objectKey =
      config.get<string>('VYBEKART_EMAIL_LOGO_PATH')?.trim() ||
      'vybekart_logo.png';
    const key = objectKey.replace(/^\/+/, '');
    return normalizeLogoUrlForEmail(
      `${supabase}/storage/v1/object/public/${encodeURIComponent(bucket)}/${key}`,
    );
  }
...
export function getVybeKartMailBranding(config: ConfigService): VybeKartMailBranding {
  return {
    ...
    logoUrl: resolveEmailLogoUrl(config),
...
<body class="vk-body" style="margin:0;padding:0;background-color:${VYBE_THEME.bgLight};color-scheme:light only;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;-webkit-font-smoothing:antialiased;">
  <table role="presentation" class="vk-body" width="100%" cellspacing="0" cellpadding="0" bgcolor="${VYBE_THEME.bgLight}" style="background-color:${VYBE_THEME.bgLight};padding:28px 12px;">
    <tr><td align="center">
      <table role="presentation" class="vk-card" width="600" cellspacing="0" cellpadding="0" bgcolor="${VYBE_THEME.surfaceLight}" style="max-width:600px;width:100%;background-color:${VYBE_THEME.surfaceLight};border-radius:16px;overflow:hidden;box-shadow:0 8px 32px rgba(11,30,91,0.12);">
        ...
        <tr>
          <td class="vk-content" bgcolor="${VYBE_THEME.surfaceLight}" style="padding:32px 28px;background-color:${VYBE_THEME.surfaceLight};color:${VYBE_THEME.textLight};">
            ...
            <p class="vk-muted" style="margin:16px 0 0;font-size:13px;line-height:1.65;color:${VYBE_THEME.textMutedLight};">
              ...
              &nbsp;·&nbsp;
              <a class="vk-link" href="${escapeHtml(b.websiteUrl)}" style="color:${VYBE_THEME.primary};text-decoration:none;">vybekart.co.in</a>
            </p>
            <p class="vk-muted" style="margin:14px 0 0;font-size:12px;line-height:1.6;color:${VYBE_THEME.textMutedLight};">
              <a class="vk-link" href="${escapeHtml(b.termsUrl)}" style="color:${VYBE_THEME.textMutedLight};text-decoration:underline;">Terms &amp; Conditions</a>
              &nbsp;·&nbsp;
