...
2. In [Render](https://render.com), go to **Dashboard → New → Blueprint**.
...
4. After the first deploy, in the **Web Service → Environment** tab, add any optional variables (e.g. `LIVEKIT_URL`, `LIVEKIT_API_KEY`, `LIVEKIT_API_SECRET`, `MAIL_*`, `CORS_ORIGIN`). `JWT_SECRET` and `JWT_REFRESH_SECRET` are auto-generated by the blueprint; you can replace them with your own if needed.
5. Migrations run on each start (`npx prisma migrate deploy` in the start command), so schema updates deploy automatically. Run **Manual Deploy** if you need to redeploy.
...
The service uses a **Dockerfile** so the build runs inside the image and `dist/` is always present at runtime (avoids "dist/main.js not found" on Render's native Node runtime).
